...
class Drone:
    ...
    def at_operating_altitude(self):
        return abs(self.target_position.z - self.current_position.z) <= 0.05
    
    def at_target_xy(self):
        return abs(self.target_position.x - self.current_position.x) <= 0.05 and abs(self.target_position.y - self.current_position.y) <= 0.05
    ...
    def set_state_target(self):
        if self.mission_state == MissionState.TAKEOFF:
            ...
            target = Point(self.current_position.x, self.current_position.y, 0.0)
    ...
    def fly(self, event):
        ...
        if self.mission_state == MissionState.LANDED:
            ...
            self.command.forxe.x, self.command.force.y = 0.0, 0.0
